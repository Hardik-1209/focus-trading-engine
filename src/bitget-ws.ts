...
export class BitgetWS extends EventEmitter {
  ...
  private handleMessage(msg: any) {
    ...
    if (channel === 'ticker') {
      ...
      for (const d of msg.data) {
        const candle = this.parseCandle(d);
        if (candle) this.upsertCandle(this.candle5mBuffer, candle);
      }
  ...
  private parseCandle(d: string[]): Candle | null {
  ...
  private upsertCandle(buf: Candle[], candle: Candle) {
