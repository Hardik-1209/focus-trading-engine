...
import { resetInMemoryStatus, logHealth } from './supabase-logger';
import { clearActivePositionsInMemory } from './position-guardian';
 ...
 */
export async function executeCleanSlateReset(
  targetVersion: string = CONFIG.VERSION,
  force = false
): Promise<ResetResult> {
  ...
  try {
    ...
    riskGovernor.resetForNewVersion(10.00);
    clearActivePositionsInMemory();
    resetInMemoryStatus(targetVersion);
    ...
    const { error: statusErr } = await supabase.from('engine_status').upsert({
      ...
      mode: CONFIG.DRY_RUN ? `DRY_RUN (v${targetVersion})` : `LIVE (v${targetVersion})`,
      updated_at: now,
      live_indicators: {
        price: 0,
 ...
 */
export async function checkAndExecuteVersionReset(): Promise<boolean> {
  try {
    console.log(`[VersionManager] 🔍 Verifying version lifecycle state for v${CONFIG.VERSION}...`);
    ...
    const cleanVer = CONFIG.VERSION.replace(/\./g, '');
    ...
    const versionMismatch = storedVersion !== CONFIG.VERSION;
    const sessionMismatch = !activeSession || activeSession.id !== expectedSessionId;

    if (versionMismatch || sessionMismatch) {
      console.log(
        `[VersionManager] 🚀 New engine release detected! ` +
        `Current: v${CONFIG.VERSION} | Stored: ${storedVersion || 'none'} | Active Session: ${activeSession?.id || 'none'}`
      );
      console.log(`[VersionManager] 🧹 Triggering automatic Clean Slate reset...`);
      await executeCleanSlateReset(CONFIG.VERSION, true);
      return true;
    }

    console.log(`[VersionManager] ✅ Version v${CONFIG.VERSION} is current and active (Session: ${activeSession.id}).`);
    return false;
  } catch (err: any) {
