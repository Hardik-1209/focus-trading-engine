...
class RiskGovernor {
    symbolStreaks = new Map();
    dailyStartingBalance = 10.00;
    dailyRealizedPnl = 0;
    dailyHalted = false;
    dailyResetDate = new Date().getUTCDate();
    ...
    canTradeSymbol(symbol) {
        ...
        if (this.dailyHalted) {
            return {
                allowed: false,
                reason: `DAILY_KILL_SWITCH_ACTIVE: Account reached ${(config_1.RISK.DAILY_MAX_DRAWDOWN_PCT * 100).toFixed(1)}% daily max loss. Trading halted until next UTC day.`,
            };
        }
        const drawdownPct = Math.abs(Math.min(0, this.dailyRealizedPnl)) / Math.max(1, this.dailyStartingBalance);
        if (drawdownPct >= config_1.RISK.DAILY_MAX_DRAWDOWN_PCT) {
            this.dailyHalted = true;
            console.warn(`[Risk Governor] 🚨 DAILY KILL SWITCH TRIGGERED: Daily loss -$${Math.abs(this.dailyRealizedPnl).toFixed(4)} exceeded ${(config_1.RISK.DAILY_MAX_DRAWDOWN_PCT * 100).toFixed(1)}% of wallet. Halting all new trades.`);
            return {
                allowed: false,
     ...
     */
    recordTradeOutcome(symbol, pnl, isWin) {
        this.checkDayRoll();
        ...
        if (isWin) {
            ...
            console.warn(`[Risk Governor] ⚠️ ${symbol} LOSS recorded (-$${Math.abs(pnl).toFixed(4)}). Streak: ${streak.consecutiveLosses}/${config_1.RISK.CONSECUTIVE_LOSS_LIMIT}`);
            if (streak.consecutiveLosses >= config_1.RISK.CONSECUTIVE_LOSS_LIMIT) {
                streak.cooldownUntil = Date.now() + config_1.RISK.SYMBOL_COOLDOWN_MS;
                const cooldownHours = (config_1.RISK.SYMBOL_COOLDOWN_MS / 3600000).toFixed(1);
...
exports.riskGovernor = new RiskGovernor();
//# sourceMappingURL=risk-governor.js.map